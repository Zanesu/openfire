...
/**
 * Representation of users interacting with the chat service. A user may join
 * serveral rooms hosted by the chat service. That means that we are going to
 * have an instance of this class for the user and several MUCRoles for each
 * joined room.
 * <p>
 *
 * This room occupant is being hosted by this JVM. When the room occupant is
 * hosted by another cluster node then an instance of {@link RemoteMUCRole} will
 * be used instead.
 * 
...
public class LocalMUCUser implements MUCUser {
	 ...
	 * @param packetRouter
	 *            the router for sending packets from this user.
	 * @param jid
	 ...
	 *
	 * @param roomName
	 *            The name of the room we're being removed
	 */
	...
	/**
	 * Generate a conflict packet to indicate that the nickname being
	 * requested/used is already in use by another user.
	 * 
	 * @param packet
	 *            the packet to be bounced.
	 * @param error
	...
	/**
	 * This method does all packet routing in the chat server. Packet routing is
	 * actually very simple:
	 * 
	 * <ul>
	 * <li>Discover the room the user is talking to (server packets are dropped)
	 * </li>
	 * <li>If the room is not registered and this is a presence "available"
	 * packet, try to join the room</li>
	 * <li>If the room is registered, and presence "unavailable" leave the room
	 * </li>
	...
	public void process(Message packet) {
		...
		if (group == null) {
			...
			// In the future, we'll need to support TYPE_IQ queries to the
			// server for MUC
			...
			if (role == null) {
				if (server.hasChatRoom(group)) {
					...
					if (Message.Type.normal == packet.getType()) {
						// An user that is not an occupant could be declining an
						// invitation
						...
						if (userInfo != null && userInfo.element("decline") != null) {
							...
							// WARNING: Potential fraud if someone fakes the
							// "from" of the
							// message with the JID of a member and sends a
							// "decline"
				...
				if (!role.getUserAddress().equals(packet.getFrom())) {
					...
					try {
						if (role.getChatRoom().getRoomHistory().isSubjectChangeRequest(packet)) {
							// An occupant is trying to change the room's
							// subject
							role.getChatRoom().changeSubject(packet, role);

						} else {
							// An occupant is trying to send a private, send
							// public message,
							// invite someone to the room or reject an
							// invitation
							...
							if (resource == null && Message.Type.groupchat == type) {
								...
								if (userInfo != null && userInfo.element("invite") != null) {
									...
									// Try to keep the list of extensions sent
									// together with the
									// message invitation. These extensions will
									// be sent to the
									// invitees.
									@SuppressWarnings("unchecked")
									List<Element> extensions = new ArrayList<>(packet.getElement().elements());
	...
	public void process(IQ packet) {
		...
		if (group == null) {
			...
			MUCRole role = roles.get(group);
			if (role == null) {
				// If a non-occupant sends a disco to an address of the form
				// <room@service/nick>,
				...
				// Only process IQ result packet if it's a private packet sent
				// to another
				// room occupant
				if (packet.getTo().getResource() != null) {
					try {
						// User is sending an IQ result packet to another room
						// occupant
						role.getChatRoom().sendPrivatePacket(packet, role);
					} catch (NotFoundException e) {
						// Do nothing. No error will be sent to the sender of
						// the IQ result packet
					}
				}
			} else {
				// Check and reject conflicting packets with conflicting roles
				...
				if (!role.getUserAddress().equals(packet.getFrom())) {
					...
					try {
						...
						if (query != null && "http://jabber.org/protocol/muc#owner".equals(query.getNamespaceURI())) {
							...
							if (packet.getTo().getResource() != null) {
								// User is sending an IQ packet to another room
								// occupant
								role.getChatRoom().sendPrivatePacket(packet, role);
							} else {
								sendErrorPacket(packet, PacketError.Condition.bad_request);
	...
	public void process(Presence packet) {
		...
		if (group != null) {
			...
			if (role == null || mucInfo != null) {
				// If we're not already in a room (role == null), we either are
				// joining it or it's not
				// properly addressed and we drop it silently
				// Alternative is that mucInfo is not null, in which case the
				// client thinks it isn't in the room, so we should join anyway.
				if (recipient.getResource() != null && recipient.getResource().trim().length() > 0) {
					if (packet.isAvailable()) {
						try {
							...
							// Check for password & requested history if client
							// supports MUC
							if (mucInfo != null) {
							...
							// If the client that created the room is non-MUC
							// compliant then
							...
							// If a user attempts to enter a room while it is
							// "locked" (i.e., before the room creator provides
							// an initial configuration and therefore before the
							// room officially exists), the service MUST refuse
							// entry and return an <item-not-found/> error to
							// the user
						...
						// TODO: send error message to user (can't send presence
						// to group you
						// haven't joined)
					}
				} else {
					if (packet.isAvailable()) {
						...
						// If the user does not specify a room nickname (note
						// the bare JID on the 'from' address in the following
						// example), the service MUST return a <jid-malformed/>
						// error
						sendErrorPacket(packet, PacketError.Condition.jid_malformed);
					}
					// TODO: send error message to user (can't send packets to
					// group you haven't
					// joined)
				}
			} else {
				...
				if (!role.getUserAddress().equals(packet.getFrom())) {
					...
					if (Presence.Type.unavailable == packet.getType()) {
						try {
							// TODO Consider that different nodes can be
							// creating and processing this presence at the same
							// time (when remote node went down)
							removeRole(group);
						...
						try {
							...
							if (resource == null || role.getNickname().equalsIgnoreCase(resource)) {
								...
								// Occupant has changed his nickname. Send two
								// presences
								// to each room occupant

								// Check if occupants are allowed to change
								// their nicknames
								if (!role.getChatRoom().canChangeNickname()) {
									sendErrorPacket(packet, PacketError.Condition.not_acceptable);
								}
								// Answer a conflic error if the new nickname is
								// taken
								else if (role.getChatRoom().hasOccupant(resource)) {
									sendErrorPacket(packet, PacketError.Condition.conflict);
								} else {
									// Send "unavailable" presence for the old
									// nickname
									Presence presence = role.getPresence().createCopy();
									// Switch the presence to OFFLINE
									presence.setType(Presence.Type.unavailable);
									presence.setStatus(null);
									// Add the new nickname and status 303 as
									// properties
									...
									// Send availability presence for the new
									// nickname
