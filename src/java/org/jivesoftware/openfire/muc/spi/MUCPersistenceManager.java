...
/**
 * A manager responsible for ensuring room persistence. There are different ways
 * to make a room persistent. The first attempt will be to save the room in a
 * relation database. If for some reason the room can't be saved in the database
 * an alternative repository will be used to save the room such as XML files.
 * <p>
 * 
 * After the problem with the database has been solved, the information saved in
 * the XML files will be moved to the database.
 *
...
public class MUCPersistenceManager {
	...
	// property name for optional number of days to limit persistent MUC history
	// during reload (OF-764)
	...
	private static final String LOAD_ROOM = "SELECT roomID, creationDate, modificationDate, naturalName, description, lockedDate, "
	...
	private static final String LOAD_HISTORY = "SELECT sender, nickname, logTime, subject, body FROM ofMucConversationLog "
			+ "WHERE logTime>? AND roomID=? AND (nickname IS NOT NULL OR subject IS NOT NULL) ORDER BY logTime";
	private static final String LOAD_ALL_ROOMS = "SELECT roomID, creationDate, modificationDate, name, naturalName, description, "
	...
	private static final String LOAD_ALL_AFFILIATIONS = "SELECT ofMucAffiliation.roomID,ofMucAffiliation.jid,ofMucAffiliation.affiliation "
			+ "FROM ofMucAffiliation,ofMucRoom WHERE ofMucAffiliation.roomID = ofMucRoom.roomID AND ofMucRoom.serviceID=?";
	private static final String LOAD_ALL_MEMBERS = "SELECT ofMucMember.roomID,ofMucMember.jid,ofMucMember.nickname FROM ofMucMember,ofMucRoom "
			+ "WHERE ofMucMember.roomID = ofMucRoom.roomID AND ofMucRoom.serviceID=?";
	private static final String LOAD_ALL_HISTORY = "SELECT ofMucConversationLog.roomID, ofMucConversationLog.sender, ofMucConversationLog.nickname, "
	...
	private static final String UPDATE_ROOM = "UPDATE ofMucRoom SET modificationDate=?, naturalName=?, description=?, "
	...
	private static final String ADD_ROOM = "INSERT INTO ofMucRoom (serviceID, roomID, creationDate, modificationDate, name, naturalName, "
	...
	private static final String ADD_CONVERSATION_LOG = "INSERT INTO ofMucConversationLog (roomID,sender,nickname,logTime,subject,body) "
			+ "VALUES (?,?,?,?,?,?)";
	...
	public static void loadFromDB(LocalMUCRoom room) {
		...
		try {
			...
			room.setCreationDate(new Date(Long.parseLong(rs.getString(2).trim()))); // creation
																					// date
			room.setModificationDate(new Date(Long.parseLong(rs.getString(3).trim()))); // modification
																						// date
			...
			// Recreate the history only for the rooms that have the
			// conversation logging
			// enabled
			if (room.isLogEnabled()) {
				pstmt = con.prepareStatement(LOAD_HISTORY);
				// Reload the history, using "muc.history.reload.limit" (days);
				// defaults to 2
			...
			// If the room does not include the last subject in the history then
			// recreate one if
			// possible
			if (!room.getRoomHistory().hasChangedSubject() && room.getSubject() != null
					&& room.getSubject().length() > 0) {
				room.getRoomHistory().addOldMessage(room.getRole().getRoleAddress().toString(), null,
						room.getModificationDate(), room.getSubject(), null);
			}
			...
			while (rs.next()) {
				...
				try {
					...
					case outcast:
						...
						break;
					default:
						Log.error("Unkown affiliation value " + affiliation + " for user " + affiliationJID.toBareJID()
								+ " in persistent room " + room.getID());
					}
				} catch (Exception e) {
			...
			// Set now that the room's configuration is updated in the database.
			// Note: We need to
			// set this now since otherwise the room's affiliations will be
			// saved to the database
			// "again" while adding them to the room!
			room.setSavedToDB(true);
			if (room.getEmptyDate() == null) {
				// The service process was killed somehow while the room was
				// being used. Since
				// the room won't have occupants at this time we need to set the
				// best date when
	...
	public static void saveToDB(LocalMUCRoom room) {
		...
		try {
			...
			if (room.wasSavedToDB()) {
				...
				pstmt.setLong(1, XMPPServer.getInstance().getMultiUserChatManager()
						.getMultiUserChatServiceID(room.getMUCService().getServiceName()));
				pstmt.setLong(2, room.getID());
	...
	public static void deleteFromDB(MUCRoom room) {
		...
		try {
			...
			// Update the room (in memory) to indicate the it's no longer in the
			// database.
	 ...
	 */
	public static Collection<LocalMUCRoom> loadRoomsFromDB(MultiUserChatService chatserver, Date emptyDate,
			PacketRouter packetRouter) {
		Long serviceID = XMPPServer.getInstance().getMultiUserChatManager()
				.getMultiUserChatServiceID(chatserver.getServiceName());
		...
		// Set now that the room's configuration is updated in the database.
		// Note: We need to
		// set this now since otherwise the room's affiliations will be saved to
		// the database
		// "again" while adding them to the room!
	...
	private static Map<Long, LocalMUCRoom> loadRooms(Long serviceID, Date emptyDate, MultiUserChatService chatserver,
			PacketRouter packetRouter) throws SQLException {
		...
		try {
			...
			while (resultSet.next()) {
				try {
					...
					room.setCreationDate(new Date(Long.parseLong(resultSet.getString(2).trim()))); // creation
																									// date
					room.setModificationDate(new Date(Long.parseLong(resultSet.getString(3).trim()))); // modification
																										// date
					...
					Log.error("A database exception prevented one particular MUC room to be loaded from the database.",
							e);
				}
			}
		} finally {
	...
	private static void loadHistory(Long serviceID, Map<Long, LocalMUCRoom> rooms) throws SQLException {
		...
		try {
			...
			// Reload the history, using "muc.history.reload.limit" (days) if
			// present
			long from = 0;
			String reloadLimit = JiveGlobals.getProperty(MUC_HISTORY_RELOAD_LIMIT);
			if (reloadLimit != null) {
				// if the property is defined, but not numeric, default to 2
				// (days)
				int reloadLimitDays = JiveGlobals.getIntProperty(MUC_HISTORY_RELOAD_LIMIT, 2);
			...
			while (resultSet.next()) {
				try {
					...
					// Skip to the next position if the room does not exist or
		...
		// Add the last known room subject to the room history only for those
		// rooms that still
		// don't have in their histories the last room subject
		for (MUCRoom loadedRoom : rooms.values()) {
			if (!loadedRoom.getRoomHistory().hasChangedSubject() && loadedRoom.getSubject() != null
					&& loadedRoom.getSubject().length() > 0) {
				loadedRoom.getRoomHistory().addOldMessage(loadedRoom.getRole().getRoleAddress().toString(), null,
			...
			while (resultSet.next()) {
				try {
					...
					try {
						...
						Log.warn(
								"An illegal JID ({}) was found in the database, "
										+ "while trying to load all affiliations for room " + "{}. The JID is ignored.",
					...
					try {
						...
						case outcast:
							...
							break;
						default:
							Log.error("Unknown affiliation value " + affiliation + " for user " + affiliationJID
									+ " in persistent room " + room.getID());
						}
					...
					Log.error(
							"A database exception prevented affiliations for one particular MUC room to be loaded from the database.",
							e);
				}
			}
		...
		try {
			...
			while (resultSet.next()) {
				try {
					...
					Log.error(
							"A database exception prevented members for one particular MUC room to be loaded from the database.",
							e);
				}
			}
		} finally {
	 ...
	 */
	public static void saveAffiliationToDB(MUCRoom room, JID jid, String nickname, MUCRole.Affiliation newAffiliation,
 ...
 */
	...
	/**
	 * Returns an integer based on the binary representation of the roles to
	 * broadcast.
	 * 
	 * @param room
	 *            the room to marshall its roles to broadcast.
	 * @return an integer based on the binary representation of the roles to
	 *         broadcast.
	 */
